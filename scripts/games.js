...
class Game {
    constructor(name, version, image, config){
        ...
        if (config != undefined) {
            ...
            if (this.config.platforms != undefined) this.platforms = this.config.platforms;
        }
    }
    ...
    render(){
        ...
        if (this.links != undefined) {
            ...
            for (var lin in this.links) {
                ren = ren + `<button>` +
                    (this.links[lin][1].substr(0, 4) == "Play" ? `<img src="images/gameinfo/play.png" style="width: 16px" /> ` : ``)
    ...
    renderGenre() {
        return "<img src='images/gameinfo/apptype.png' style='width: 16px'> " + this.getInfo("type").substr(0, 1).toUpperCase() + this.getInfo("type").substr(1) + "<br /><img src='images/gameinfo/genre.png' style='width: 16px'> " + this.getInfo("genre");
    }

    renderPlatforms() {
        let render = "";
        let validPlatforms = ["web_pc", "web_phone", "windows", "linux", "mobile"];
        for (let vp in this.platforms) {
            if (validPlatforms.includes(this.platforms[vp])) render = render + "<img src='images/gameinfo/available_" + this.platforms[vp] + ".png' style='width: 32px'>";
        }
        return render;
    }

    renderRating() {
        return "<img src='images/gameinfo/star.png' style='width: 16px'>".repeat(this.getInfo("rating")) + "<img src='images/gameinfo/nostar.png' style='width: 16px'>".repeat(5 - this.getInfo("rating"));
    }
...
const games = {
    ...
    scrapRPG: new Game("ScrapRPG", "1.0.7", "scraprpg.png", {
        git: "scraprpg",
